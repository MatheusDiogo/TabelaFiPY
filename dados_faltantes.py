...
def main():
    ...
    # Identificar modelos sem dados no último mês
    modelos_sem_dados = df_final[(df_final['MesReferencia'] == ultimo_mes) & (df_final['Valor'].isna())]['Modelo'].unique()
    
    print(modelos_sem_dados)

    # Excluir modelos com dados faltantes para o último mês
    df_final = df_final[~df_final['Modelo'].isin(modelos_sem_dados)]
